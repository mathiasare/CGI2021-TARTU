  ...
  methods: {
    ...
    async onButtonClick() {
      if (await this.checkInputValues()) {
        ...
        this.title =
          ...
          this.endDate +
          ".";
        this.chartData = await this.getGraphValues(
    ...
    async checkInputValues() {
      ...
      if (
        ...
        if (
          dateBeg.getFullYear() < 1800 ||
          dateBeg.getFullYear() > 2100 ||
          dateEnd.getFullYear() < 1800 ||
          dateEnd.getFullYear() > 2100
        )
          return false;
    ...
    async getGraphValues(beginning, end) {
      ...
      const days_between =
        (end.getTime() - date.getTime()) / (1000 * 60 * 60 * 24);
