<template>
  ...
  <div class="container">
    ...
    <input
      class="field"
      :class="{ invalid: invalidLatitude }"
      :readonly="fromMap"
      id="latitude"
      type="text"
      placeholder="Coordinates in EPSG:4326"
    ...
    <input
      class="field"
      v-bind:class="{ invalid: invalidLongitude }"
      :readonly="fromMap"
      id="longitude"
      type="text"
      placeholder="Coordinates in EPSG:4326"
...
export default {
    ...
    checkInputs(name) {
      if (name == "lng") {
        ...
        if (new Date(this.dateInput).toString() == "Invalid Date") {
          ...
          if (
            checkedDate.getFullYear() < 1800 ||
            checkedDate.getFullYear() > 2100
          ) {
            return false;
          }
        }
  ...
  watch: {
    longitudeInput: function () {
      if (!this.$store.getters.isFromMap) {
        if (this.longitudeInput == "") {
          //If input is empty string then change value to 0.0
    ...
    fromMap: function () {
      //Coordinates from Map checkbox bind
